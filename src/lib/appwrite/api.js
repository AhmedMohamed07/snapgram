import { account, appwriteConfig, avatars, databases, storagee } from './config';
...
export async function uploadFile(file) {
  try {
    const uploadedFile = await storagee.createFile(
...
export function getFilePreview(fileId) {
  try {
    const fileUrl = storagee.getFilePreview(
      appwriteConfig.storageId,
      fileId,
      2000,
      2000,
...
export async function deleteFile(fileId) {
  try {
    await storagee.deleteFile(appwriteConfig.storageId, fileId);
